...
class Brainup_Cielo_Model_Cc extends Brainup_Cielo_Model_Abstract
{

    protected $_code  = 'Brainup_Cielo_Cc';
    protected $_formBlockType = 'Brainup_Cielo/form_cc';
    protected $_infoBlockType = 'Brainup_Cielo/info_cc';
    ...
    public function assignData($data)
    {
		...
		if($data->getParcelsNumber() > $withoutInterest)
		{
			...
			$installmentValue = Mage::helper('Brainup_Cielo')->calcInstallmentValue
								(
									$totalValue, 
									$interestValue / 100, 
	...
	public function validate()
	{
		...
		{
			$availableTypes = Mage::getModel('Brainup_Cielo/cc_types')->getCodes();
			if(!in_array($info->getCcType(), $availableTypes))
			{
				$errorMsg = Mage::helper('Brainup_Cielo')->__('Credit card type is not allowed for this payment method.');
			}						
		}
		else
		{
			...
			if(in_array($info->getCcType(), $availableTypes))
			{
				if ($this->validateCcNum($ccNumber))
				{
					...
					{
						$errorMsg = Mage::helper('Brainup_Cielo')->__('Credit card number mismatch with credit card type.');
					}
				}
				else
				{
					$errorMsg = Mage::helper('Brainup_Cielo')->__('Invalid Credit Card Number');
				}
			...
			if ($errorMsg === false)
			{
				...
				{
					$errorMsg = Mage::helper('Brainup_Cielo')->__('Please enter a valid credit card verification number.');
				}
			}
			...
			if (!$this->_validateExpDate($info->getCcExpYear(), $info->getCcExpMonth()))
			{
				...
				$errorMsg = Mage::helper('Brainup_Cielo')->__('Incorrect credit card expiration date.');
			}
	...
	public function getOrderPlaceRedirectUrl()
	{
		...
		$value 				= Mage::helper('Brainup_Cielo')->formatValueForCielo($quote->getGrandTotal());
		...
		$webServiceOrder = Mage::getModel('Brainup_Cielo/webServiceOrder', array('enderecoBase' => $environment, 'caminhoCertificado' => $sslFile));
		...
		(
			...
			'postbackURL'		=> Mage::getUrl('brainupCielo/pay/verify'),
		...
		if($redirectUrl == false)
		{
			...
			{
				return Mage::getUrl('brainupCielo/pay/verify');
			}
			// erro nao indentificado
			else
			{
				return Mage::getUrl('brainupCielo/pay/failure');
			}
		}
		else
		{
	...
	private function _getToken($customerId, $ccType, $ccNumber)
	{
		...
		$sql = "SELECT token FROM " . $tablePrefix . "brainup_cielo_customer_token WHERE customer_id=" . $customerId . " AND " . 
	...
	private function _saveNewToken($responseXml, $customerId, $ccType)
	{
		...
		$sql = "INSERT INTO `" . $tablePrefix . "brainup_cielo_customer_token`" . 
	...
	private function _getTokenById($tokenString)
	{
		...
		$query = "SELECT token FROM " . $tablePrefix . "brainup_cielo_customer_token WHERE token_id=" . $tokenData[1] ." AND customer_id = ".$quote->getCustomerId();
		$returnValues = array();
