	...
	function brainupCieloUpdateInstallments(cardType,installments)
	{
	...
	function selectCardTypeCc(element)
	{
		...
		$('brainup-cielo-cc-type').value = $(element).getAttribute('ccType');
	...
	function brainupCieloCcMask(event)
	{	
		var field = event.currentTarget;
		
		field.maxLength=14;
		
		if(brainupCieloCcSpecialKeys(event))
		{
			return true;
		}
		
		if(!brainupCieloCcNumberKeys(event))
		{
			Event.stop(event);
			return false;
		}
	...
	function brainupSetCieloCcMask(event)
	{
		var field = event.currentTarget;
	...
	function brainupCieloCcNumberKeys(event)
	{	
		var keyCode = ('which' in event) ? event.which : event.keyCode;
	...
	function brainupCieloCcSpecialKeys(event)
	{
		var keyCode = ('which' in event) ? event.which : event.keyCode;
